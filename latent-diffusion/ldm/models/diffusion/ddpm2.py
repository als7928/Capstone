...
class DDPM(pl.LightningModule):
    ...
    def get_loss(self, pred, target, mean=True):
        if self.loss_type == 'l1':
            ...
            if mean:
                loss = loss.mean()
            #print("dddddddddddd",loss,"dddddddddddd")
        elif self.loss_type == 'l2':
            if mean:
                loss = torch.nn.functional.mse_loss(target, pred)
            else:
                loss = torch.nn.functional.mse_loss(target, pred, reduction='none')
        elif self.loss_type == 'gg':
            if mean:
                loss = torch.nn.functional.nll_loss(target, pred)
            else:
                loss = torch.nn.functional.nll_loss(target, pred, reduction='none')
        else:
...
class LatentDiffusion(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.cond_stage_key in ["image", "LR_image", "segmentation", "rgb",
